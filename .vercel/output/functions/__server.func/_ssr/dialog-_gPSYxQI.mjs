...
function useCryptoStore() {
  ...
  const addCrypto = async (crypto) => {
    ...
    const finalId = crypto.id && uuidRegex.test(crypto.id) ? crypto.id : typeof window !== "undefined" && window.crypto?.randomUUID ? window.crypto.randomUUID() : "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, (c) => {
      const r = Math.random() * 16 | 0;
      const v = c === "x" ? r : r & 3 | 8;
      return v.toString(16);
    });
    const newCrypto = { ...crypto, id: finalId };
