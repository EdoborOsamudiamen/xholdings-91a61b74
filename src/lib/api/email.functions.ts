  ...
  .handler(async ({ data: input }) => {
    ...
    if (RESEND_API_KEY) {
      try {
        ...
        if (res.ok) {
          return { ok: true, resend: result };
        }
        console.error("[sendEmailServerFn] Resend API error:", result);
      } catch (err) {
        console.error("[sendEmailServerFn] Direct Resend error:", err);
      }
    }

    // 2. Fallback to invoking the Supabase Edge Function from the server side (avoids browser CORS)
    try {
      const { data: edgeData, error: edgeError } = await supabase.functions.invoke("send-notification-email", {
        body: { to, type, data },
      });
      if (edgeError) {
        console.error("[sendEmailServerFn] Edge function error:", edgeError);
        return { ok: false, error: edgeError.message || String(edgeError) };
      }
      return { ok: true, result: edgeData };
    } catch (err) {
      console.error("[sendEmailServerFn] Fallback invocation error:", err);
